...
def run_game(g, env_name, multi_part_agent_ids, actions_spaces, policy_list, render_mode):
    ...
    while not g.is_terminal():
        ...
        all_observes, reward, done, info_before, info_after, attri_dict = g.step(joint_act)
        if env_name.split("-")[0] in ["magent"]:
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--my_ai", default="td3_lstm", help="random")  # agent 1
    # parser.add_argument("--opponent", default="random", help="random")  # agent 0
    ...
    # from agents.td3_lstm.my_submission import run
    # from agents.random.submission import my_controller as enemy_controller
    from agents.rule.collect_exp import run

    run(game, actions_space)
